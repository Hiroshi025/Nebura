...
const EvalCommand: Precommand = {
  ...
  descriptionLocalizations: {
    "en-US": "Evaluates JavaScript code with advanced features",
    "es-ES": "Evalúa código JavaScript con características avanzadas",
  },
  ...
  async execute(client, message, args, prefix) {
    ...
    if (!code) {
      return await sendError(message, client.t("discord:eval.noCode", { lng: lang }), prefix);
    }
    ...
    try {
      ...
      return await sendResponse(message, {
        ...
        evaluated,
        flags,
        successTitle: client.t("discord:eval.successTitle", { lng: lang }),
        successDesc: client.t("discord:eval.successDesc", { lng: lang, executionTimeMs, executionTimeSec }),
      });
    } catch (error: any) {
      ...
      return await sendError(
        message,
        client.t("discord:eval.error", { lng: lang, error: error?.message || error }),
        prefix,
        code,
      );
    }
