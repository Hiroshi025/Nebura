...
import { createEvent } from "../../../structure/utils/builders";

/**
 * Handles the creation of messages in the Discord server.
 */
      ...
      return message.channel.send({
        embeds: [
          new ErrorEmbed()
            ...
            .setDescription(
              [
                `${client.getEmoji(message.guild.id, "error")} ${client.t("discord:errors.clientDataDesc", { lng: language })}`,
                client.t("discord:errors.setupInstruction", { lng: language, prefix }),
              ].join("\n"),
            ),
        ],
      });
