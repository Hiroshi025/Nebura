"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="f8aefff9-c78d-5c04-a380-9e58bc9ce094")}catch(e){}}();
...
const user_1 = require("../../../adapters/validators/user");
const auth_repositories_1 = require("../../../gateaway/auth.repositories");
...
class AuthService {
    ...
    authRepository = new auth_repositories_1.AuthRepository(); // Instancia del repositorio de autenticación
    /**
    ...
    async createAuth(body) {
        try {
            ...
            if (existingUser) {
                ...
                await this.notifier.sendWebhookNotification("User Registration Attempt Failed", `A registration attempt failed because the email \`${email}\` is already in use.`, "#F44336", [
            ...
            if (!passwordHash) {
                ...
                await this.notifier.sendWebhookNotification("User Registration Failed", `Password encryption failed for email: \`${email}\`.`, "#F44336", [{ name: "Email", value: email, inline: true }], { content: "🔴 Registration failed (encryption)", username: "Auth Service" });
                return { error: "ENCRYPTION_ERROR", message: "Failed to encrypt password" };
            }
            const response = await axios_1.default.get(`https://discord.com/api/v10/users/${discordId}`, {
                headers: {
            ...
            if (!newUser) {
                await this.notifier.sendWebhookNotification("User Registration Failed", `User registration failed for email: \`${email}\`.`, "#F44336", [{ name: "Email", value: email, inline: true }], { content: "🔴 Registration failed", username: "Auth Service" });
            }
            await this.notifier.sendWebhookNotification("User Registration Successful", `A new user has been successfully registered with email: \`${email}\`.`, "#4CAF50", [
                { name: "Email", value: email, inline: true },
                { name: "Name", value: name, inline: true },
            ...
            await this.notifier.sendWebhookNotification("User Registration Error", `An error occurred during user registration.`, "#F44336", [
    ...
    async loginAuth(credentials) {
        try {
            ...
            // Validar estructura de datos
            const validation = user_1.AuthLogin.safeParse({ email, password });
            if (!validation.success) {
                return {
                    error: "VALIDATION_ERROR",
...
//# debugId=f8aefff9-c78d-5c04-a380-9e58bc9ce094
