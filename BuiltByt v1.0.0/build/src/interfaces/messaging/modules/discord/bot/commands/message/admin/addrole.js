"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="ecc969f8-4823-5829-8e64-7748c46805ad")}catch(e){}}();
...
const AddRoleCommand = {
    ...
    permissions: [discord_js_1.PermissionFlagsBits.ManageRoles],
    async execute(client, message) {
        ...
        const roleSelectRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.RoleSelectMenuBuilder()
            .setCustomId("role_select")
            .setPlaceholder("Select up to 5 roles")
            .setMinValues(1)
            .setMaxValues(5));
        const targetSelectRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
            .setCustomId("target_select")
            .setPlaceholder("Select the target")
            .addOptions({ label: "All members", value: "all" }, { label: "Only human users", value: "users" }, { label: "Only bots", value: "bots" }));
        const optionsSelectRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
            .setCustomId("options_select")
            .setPlaceholder("Additional options")
            .addOptions({ label: "Toggle Skip Existing", value: "toggle_skip" }, { label: "Set Log Channel", value: "set_log" }));
        const delayButtonsRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId("delay_1").setLabel("1s").setStyle(discord_js_1.ButtonStyle.Secondary), new discord_js_1.ButtonBuilder().setCustomId("delay_2").setLabel("2s").setStyle(discord_js_1.ButtonStyle.Secondary), new discord_js_1.ButtonBuilder().setCustomId("delay_5").setLabel("5s").setStyle(discord_js_1.ButtonStyle.Secondary));
        const actionButtonsRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            .setCustomId("confirm")
            .setLabel("Confirm Configuration")
            .setStyle(discord_js_1.ButtonStyle.Success)
            .setDisabled(config.roles.length === 0), new discord_js_1.ButtonBuilder().setCustomId("edit_roles").setLabel("Edit Roles").setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId("cancel").setLabel("Cancel").setStyle(discord_js_1.ButtonStyle.Danger));
            ...
            try {
                if (interaction.user.id !== message.author.id) {
                    return interaction.reply({
                        embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Only the command author can interact with this menu.")],
                        flags: "Ephemeral",
                    });
                }
        ...
        async function handleConfirmation(interaction) {
            ...
            try {
                ...
                if (rolesToAssign.length === 0) {
                    await interaction.editReply({
                        embeds: [new embeds_extend_1.ErrorEmbed().setTitle("Error").setDescription("No valid roles were selected for assignment.")],
                    });
                    return;
                }
                ...
                const confirmationEmbed = new embeds_extend_1.EmbedCorrect()
                ...
                const confirmButtons = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                    .setCustomId("final-confirm-addrole")
                    .setLabel("Confirm Assignment")
                    .setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId("final-cancel-addrole").setLabel("Cancel").setStyle(discord_js_1.ButtonStyle.Danger));
                await interaction.editReply({
                ...
                finalCollector.on("collect", async (finalInteraction) => {
                    try {
                        if (finalInteraction.user.id !== message.author.id) {
                            await finalInteraction.reply({
                                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Only the command author can confirm this action.")],
                                flags: "Ephemeral",
                            });
                            return;
                        }
                        ...
                        if (finalInteraction.customId === "final-confirm-addrole") {
                            const progressEmbed = new embeds_extend_1.EmbedCorrect()
                                .setTitle("Assigning Roles...")
                                .setDescription(`Progress: 0/${members.size} (0%)`)
                                .addFields({ name: "Successes", value: "0", inline: true }, { name: "Failures", value: "0", inline: true }, { name: "Skipped", value: "0", inline: true });
                                ...
                                const batchPromises = currentBatch.map(async (member, index) => {
                                    ...
                                    if (index % 5 === 0 || index === currentBatch.length - 1) {
                                        ...
                                        progressEmbed
                                            ...
                                            .spliceFields(0, 3, { name: "Successes", value: successCount.toString(), inline: true }, { name: "Failures", value: failCount.toString(), inline: true }, { name: "Skipped", value: skipCount.toString(), inline: true });
                                ...
                                try {
                                    ...
                                    if (logChannel) {
                                        const logEmbed = new discord_js_1.EmbedBuilder()
                                            ...
                                            value: rolesToAssign.map((r) => r.toString()).join(", "),
                                            inline: false,
                                        }, { name: "Target Members", value: members.size.toString(), inline: true }, { name: "Successes", value: successCount.toString(), inline: true }, { name: "Failures", value: failCount.toString(), inline: true }, { name: "Skipped", value: skipCount.toString(), inline: true })
                                            .setTimestamp();
...
//# debugId=ecc969f8-4823-5829-8e64-7748c46805ad
