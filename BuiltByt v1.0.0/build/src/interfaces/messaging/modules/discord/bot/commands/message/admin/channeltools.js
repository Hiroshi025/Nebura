"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="224d9bcc-15ae-51a4-973b-dbe7be7375a3")}catch(e){}}();
...
const ToolsChannel = {
    ...
    botpermissions: [discord_js_1.PermissionFlagsBits.ManageChannels, discord_js_1.PermissionFlagsBits.ManageMessages],
    permissions: [discord_js_1.PermissionFlagsBits.ManageChannels],
    async execute(client, message) {
        console.log("DEBUG: Channel tools command initiated");
        if (!message.guild)
            return;
        if (!message.guild || !message.channel || !(message.channel instanceof discord_js_1.TextChannel)) {
        ...
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                value: [
                    ...
                    `• Locked: ${message.channel.permissionsFor(message.guild.roles.everyone)?.has(discord_js_1.PermissionFlagsBits.SendMessages) ? "No" : "Yes"}`,
                    `• NSFW: ${message.channel.nsfw ? "Yes" : "No"}`,
                    `• Type: ${discord_js_1.ChannelType[message.channel.type]}`,
                ].join("\n"),
                inline: true,
            }, {
                name: "Statistics",
            ...
            const actionRow1 = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                ...
                .setLabel(message.channel.permissionsFor(message.guild.roles.everyone)?.has(discord_js_1.PermissionFlagsBits.SendMessages)
                ? "Lock"
                : "Unlock")
                .setStyle(message.channel.permissionsFor(message.guild.roles.everyone)?.has(discord_js_1.PermissionFlagsBits.SendMessages)
                ? discord_js_1.ButtonStyle.Danger
                : discord_js_1.ButtonStyle.Success)
                .setEmoji(message.channel.permissionsFor(message.guild.roles.everyone)?.has(discord_js_1.PermissionFlagsBits.SendMessages)
                ? "🔒"
                : "🔓"), new discord_js_1.ButtonBuilder().setCustomId("purge").setLabel("Purge Messages").setStyle(discord_js_1.ButtonStyle.Danger).setEmoji("🗑️"), new discord_js_1.ButtonBuilder().setCustomId("info").setLabel("Channel Info").setStyle(discord_js_1.ButtonStyle.Secondary).setEmoji("ℹ️"));
            // Secondary action buttons
            const actionRow2 = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                .setCustomId("nsfw_toggle")
                .setLabel(message.channel.nsfw ? "Make SFW" : "Make NSFW")
                .setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId("rename").setLabel("Rename Channel").setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId("clone").setLabel("Clone Channel").setStyle(discord_js_1.ButtonStyle.Success));
                ...
                try {
                    ...
                    switch (interaction.customId) {
                        case "slowmode":
                            ...
                            const slowmodeRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
                                .setCustomId("slowmode_select")
                                .setPlaceholder("Select slowmode duration")
                                .addOptions(slowmodeOptions.map((opt) => new discord_js_1.StringSelectMenuOptionBuilder()
                                .setLabel(opt.label)
                                .setValue(opt.value)
                        ...
                        case "lock_toggle":
                            ...
                            actionRow1.components[1]
                                ...
                                .setStyle(isLocked ? discord_js_1.ButtonStyle.Danger : discord_js_1.ButtonStyle.Success)
                                .setEmoji(isLocked ? "🔒" : "🔓");
                            await interaction.editReply({
                                content: isLocked
                        ...
                        case "purge":
                            ...
                            const purgeRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
                                .setCustomId("purge_select")
                                .setPlaceholder("Select amount to delete")
                                .addOptions(purgeOptions.map((opt) => new discord_js_1.StringSelectMenuOptionBuilder()
                                .setLabel(opt.label)
                                .setValue(opt.value)
                                .setDescription(opt.description))));
                            await interaction.editReply({
                        ...
                        case "rename":
                            ...
                            nameCollector.on("collect", async (m) => {
                                ...
                                try {
                                    ...
                                    await m.reply(`❌ Failed to rename channel: ${error instanceof Error ? error.message : "Unknown error"}`);
                                }
                                finally {
...
module.exports = ToolsChannel;
//# sourceMappingURL=channeltools.js.map
//# debugId=224d9bcc-15ae-51a4-973b-dbe7be7375a3
