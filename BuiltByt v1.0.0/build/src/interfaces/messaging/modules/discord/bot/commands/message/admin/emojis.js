"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="57d5dbdc-5381-5267-9d79-5d8528c6648c")}catch(e){}}();
 ...
 */
async function handleEmojiAdd(message, args, prefix) {
    if (!message.guild || !message.channel || message.author.bot || message.channel.type !== discord_js_1.ChannelType.GuildText)
        return;
    ...
    const existingEmoji = message.guild.emojis.cache.find((e) => e.name === finalName);
    if (existingEmoji) {
        return message.channel.send({
            embeds: [
                new embeds_extend_1.ErrorEmbed()
                    ...
                    .addFields({ name: "Existing Emoji", value: existingEmoji.toString(), inline: true }, {
                    name: "Created At",
    ...
    const emojiSlots = message.guild.premiumTier === 0 // GuildPremiumTier.None
        ? 50
    ...
    const confirmationEmbed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "Name", value: finalName, inline: true }, { name: "Type", value: isAnimated ? "Animated" : "Static", inline: true }, { name: "Preview", value: `[View Image](${emojiUrl})`, inline: true })
        .setThumbnail(emojiUrl);
    const confirmationMessage = await message.channel.send({
        embeds: [confirmationEmbed],
        components: [
            new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId("emoji-add-confirm").setLabel("Confirm").setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId("emoji-add-cancel").setLabel("Cancel").setStyle(discord_js_1.ButtonStyle.Danger)),
        ],
    });
                ...
                await interaction.update({
                    embeds: [
                        new embeds_extend_1.EmbedCorrect()
                            ...
                            .addFields({ name: "Name", value: createdEmoji.name ?? "Unknown", inline: true }, { name: "ID", value: createdEmoji.id, inline: true }, { name: "Usage", value: `\`${createdEmoji.toString()}\``, inline: true })
 ...
 */
async function handleEmojiInfo(message, args, prefix) {
    const emojiInput = args[0];
    if (!emojiInput) {
        return message.channel.send({
            embeds: [
    ...
    if (!customEmojiMatch && !isDefaultEmoji) {
        return message.channel.send({
            embeds: [
                new embeds_extend_1.ErrorEmbed().setTitle("Invalid Emoji").setDescription("Please provide a valid emoji (custom or default)."),
            ],
        });
    }
    if (customEmojiMatch) {
        ...
        const infoEmbed = new discord_js_1.EmbedBuilder()
            ...
            .setColor("Blue")
            .setThumbnail(emojiUrl)
            .addFields({ name: "Name", value: `\`${emojiName}\``, inline: true }, { name: "ID", value: `\`${emojiId}\``, inline: true }, { name: "Type", value: isAnimated ? "Animated" : "Static", inline: true }, { name: "In This Server", value: isInServer ? "✅ Yes" : "❌ No", inline: true }, {
            name: "Created At",
            value: `<t:${Math.floor(createdAt.getTime() / 1000)}:F>`,
            inline: true,
        }, { name: "URL", value: `[Click Here](${emojiUrl})`, inline: true }, { name: "Identifier", value: `\`${fullMatch}\`` });
        if (isInServer) {
            const emoji = message.guild?.emojis.cache.get(emojiId);
            infoEmbed.addFields({ name: "Managed", value: emoji?.managed ? "✅ Yes" : "❌ No", inline: true }, { name: "Available", value: emoji?.available ? "✅ Yes" : "❌ No", inline: true }, {
        ...
        const infoEmbed = new discord_js_1.EmbedBuilder()
            ...
            .setColor("Blue")
            .setThumbnail(emojiUrl)
            .addFields({ name: "Emoji", value: emojiInput, inline: true }, { name: "Unicode", value: `\`\\u{${emojiCodePoints}}\``, inline: true }, { name: "URL", value: `[Twemoji Image](${emojiUrl})`, inline: true });
        await message.channel.send({ embeds: [infoEmbed] });
    }
    return;
}
 ...
 */
async function handleEmojiJumbo(message, args, prefix) {
    if (!message.guild || !message.channel || message.author.bot || message.channel.type !== discord_js_1.ChannelType.GuildText)
        return;
    const emojiInput = args[0];
    if (!emojiInput) {
    ...
    if (customEmojiMatch) {
        ...
        const extension = isAnimated ? "gif" : "png";
        const emojiUrl = `https://cdn.discordapp.com/emojis/${emojiId}.${extension}`;
        const jumboEmbed = new discord_js_1.EmbedBuilder().setTitle(`Jumbo Emoji: ${emojiName}`).setImage(emojiUrl).setColor("Random");
        ...
        const emojiCodePoints = Array.from(emojiInput)
            ...
            .join("-");
        const emojiUrl = `https://twemoji.maxcdn.com/v/latest/72x72/${emojiCodePoints}.png`;
        const jumboEmbed = new discord_js_1.EmbedBuilder()
 ...
 */
async function handleEmojiList(client, message, args) {
    ...
    if (emojis.size === 0) {
        return message.channel.send({
            embeds: [
                new embeds_extend_1.ErrorEmbed().setTitle("No Emojis Found").setDescription("This server doesn't have any custom emojis."),
            ],
        });
    }
    ...
    const listEmbed = new discord_js_1.EmbedBuilder()
        ...
        .setColor("Blue")
        .setFooter({
        text: `Total emojis: ${emojis.size} • Animated: ${emojis.filter((e) => e.animated).size}`,
    });
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
        ...
        .setLabel("Next")
        .setStyle(discord_js_1.ButtonStyle.Primary)
        .setDisabled(page >= totalPages), new discord_js_1.ButtonBuilder().setCustomId("emoji-list-close").setLabel("Close").setStyle(discord_js_1.ButtonStyle.Danger));
 ...
 */
async function handleEmojiDelete(message, args) {
    if (!message.guild || !message.channel || message.channel.type !== discord_js_1.ChannelType.GuildText)
        return;
    ...
    let emoji = message.guild.emojis.cache.get(emojiInput);
    if (!emoji) {
        emoji = message.guild.emojis.cache.find((e) => e.name?.toLowerCase() === emojiInput.toLowerCase() || e.toString() === emojiInput);
    }
    if (!emoji) {
        return message.channel.send({
            embeds: [new embeds_extend_1.ErrorEmbed().setTitle("Emoji Not Found").setDescription("Couldn't find that emoji in this server.")],
        });
    }
    // Create confirmation embed
    const confirmationEmbed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "Emoji", value: emoji.toString(), inline: true }, { name: "ID", value: emoji.id, inline: true }, {
        name: "Created At",
    ...
    const confirmationMessage = await message.channel.send({
        embeds: [confirmationEmbed],
        components: [
            new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId("emoji-delete-confirm").setLabel("Delete").setStyle(discord_js_1.ButtonStyle.Danger), new discord_js_1.ButtonBuilder().setCustomId("emoji-delete-cancel").setLabel("Cancel").setStyle(discord_js_1.ButtonStyle.Secondary)),
        ],
    });
    const collector = confirmationMessage.createMessageComponentCollector({
        filter: (i) => i.user.id === message.author.id,
    ...
    collector.on("collect", async (interaction) => {
        if (interaction.customId === "emoji-delete-confirm") {
            try {
                ...
                await interaction.update({
                    embeds: [
                        new embeds_extend_1.ErrorEmbed().setTitle("Deletion Failed").setDescription("An error occurred while deleting the emoji."),
                    ],
                    components: [],
                });
            }
 ...
 */
async function handleEmojiRename(message, args) {
    if (!message.guild || !message.channel || message.channel.type !== discord_js_1.ChannelType.GuildText)
        return;
    const emojiInput = args[0];
    ...
    if (message.guild.emojis.cache.some((e) => e.name?.toLowerCase() === newName.toLowerCase())) {
        return message.channel.send({
            embeds: [
                new embeds_extend_1.ErrorEmbed()
    ...
    try {
        ...
        await message.channel.send({
            embeds: [
                new embeds_extend_1.EmbedCorrect()
                    ...
                    .addFields({ name: "Old Name", value: oldName ?? "Unknown", inline: true }, { name: "New Name", value: newName, inline: true }, { name: "Emoji", value: emoji.toString(), inline: true })
                    .setThumbnail(emoji.imageURL()),
            ],
        });
    }
        ...
        message.channel.send({
            embeds: [
                new embeds_extend_1.ErrorEmbed().setTitle("Rename Failed").setDescription("An error occurred while renaming the emoji."),
            ],
        });
    }
    return;
 ...
 */
async function showEmojiHelp(message, prefix) {
    if (!message.guild || !message.channel || message.channel.type !== discord_js_1.ChannelType.GuildText)
        return;
...
//# debugId=57d5dbdc-5381-5267-9d79-5d8528c6648c
