"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="4838f0d5-5d27-5942-b88d-665cc3dce4f2")}catch(e){}}();
...
exports.default = new builders_1.Event("voiceStateUpdate", async (oldState, newState) => {
    ...
    console.debug(`[voiceStateUpdate] Old channel: ${oldChannel?.id ?? "none"}, New channel: ${newChannel?.id ?? "none"}`);
    // Retrieve guild configuration from the database
    const data = await main_1.main.prisma.myGuild.findFirst({ where: { guildId: guild.id } });
    if (!data) {
    ...
    if (oldChannel !== newChannel && newChannel && newChannel.id === joinToCreate) {
        console.debug(`[voiceStateUpdate] User ${user.username}#${user.discriminator} joined the joinToCreate channel (${joinToCreate}). Creating channels...`);
        /**
        ...
        const voiceChannel = await guild.channels
            ...
            throw new error_extend_1.DiscordError("Failed to create the voice channel");
        });
        ...
        const textChannel = await guild.channels
            ...
            throw new error_extend_1.DiscordError("Failed to create the text channel");
        });
        ...
        return member?.voice
            ...
            throw new error_extend_1.DiscordError("Failed to move the member to the voice channel");
        });
    }
    /**
    ...
    if (ownedChannel &&
        ...
        console.debug(`[voiceStateUpdate] User ${member?.id} left their owned voice channel (${ownedChannel.voiceChannelId}). Deleting channels...`);
        ...
        oldChannel
            ...
            throw new error_extend_1.DiscordError("Failed to delete the voice channel");
        });
        ...
        textChannel
            ...
            throw new error_extend_1.DiscordError("Failed to delete the text channel");
        });
    }
    return;
});
//# sourceMappingURL=voiceStateUpdate.js.map
//# debugId=4838f0d5-5d27-5942-b88d-665cc3dce4f2
