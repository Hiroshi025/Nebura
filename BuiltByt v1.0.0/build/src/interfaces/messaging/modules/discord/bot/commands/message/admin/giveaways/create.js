"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="8f09c911-65ef-59b1-a0c2-67a4aebab182")}catch(e){}}();

// src/commands/giveaway/create.ts
const discord_js_1 = require("discord.js");
const main_1 = require("../../../../../../../../../main"); // Ajusta la ruta según tu estructura
const embeds_extend_1 = require("../../../../../../../../../shared/adapters/extends/embeds.extend");
//TODO: No crea el sorteo configurado y cambia los ephemerañ: true por los flags
const GiveawayCreate = {
    ...
    async execute(client, message) {
        if (!message.guild || !message.channel) {
            await message.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("This command can only be used in a server channel.")],
        ...
        const setupEmbed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription("Please configure your giveaway using the options below.\n\n**Required Steps:**\n1. Set Prize & Winners\n2. Set Duration\n3. Select Channel\n\n*Optional:*\n- Set Participation Requirements")
            .setColor("#FFA500")
            .setFooter({ text: "Giveaway creation will timeout after 10 minutes of inactivity." });
        const setupButtons = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            ...
            try {
                ...
                if (buttonInteraction.replied || buttonInteraction.deferred) {
                    await buttonInteraction.followUp({
                        embeds: [new embeds_extend_1.ErrorEmbed().setDescription("An error occurred while processing your request.")],
                    });
                }
                else {
        ...
        client.on("interactionCreate", async (modalInteraction) => {
            if (!modalInteraction.isModalSubmit())
                return;
            if (!modalInteraction.customId.startsWith("giveaway_"))
                return;
            if (modalInteraction.user.id !== message.author.id)
                return;
            try {
                ...
                await modalInteraction.reply({
                    embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Failed to process your input.")],
                    flags: "Ephemeral",
                });
            }
        });
...
// ========== Helper Functions ==========
async function handlePrizeSetup(interaction, _giveawayData) {
    const modal = new discord_js_1.ModalBuilder().setCustomId("giveaway_prize_modal").setTitle("Giveaway Prize Configuration");
    const prizeInput = new discord_js_1.TextInputBuilder()
    ...
    const winnersInput = new discord_js_1.TextInputBuilder()
        ...
        .setMinLength(1)
        .setMaxLength(2);
    modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(prizeInput), new discord_js_1.ActionRowBuilder().addComponents(winnersInput));
    await interaction.showModal(modal);
}
async function handleDurationSetup(interaction, _giveawayData) {
    const modal = new discord_js_1.ModalBuilder().setCustomId("giveaway_duration_modal").setTitle("Giveaway Duration Configuration");
    const durationInput = new discord_js_1.TextInputBuilder()
        ...
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setRequired(true);
    modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(durationInput));
    await interaction.showModal(modal);
}
async function handleChannelSelect(interaction, giveawayData) {
    ...
    channelCollector.on("collect", async (channelInteraction) => {
        ...
        if (!channel?.isTextBased()) {
            await channelInteraction.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Please select a valid text channel.")],
                flags: "Ephemeral",
            });
            return;
        }
        ...
        if (!permissions?.has(["ViewChannel", "SendMessages", "EmbedLinks"])) {
            await channelInteraction.reply({
                embeds: [
                    new embeds_extend_1.ErrorEmbed().setDescription(`I don't have permission to send messages in ${channel}. ` +
        ...
        await channelInteraction.reply({
            embeds: [new embeds_extend_1.EmbedCorrect().setDescription(`✅ Giveaway will be posted in ${channel}`)],
            flags: "Ephemeral",
        });
        channelCollector.stop();
    });
}
async function handleRequirementsSetup(interaction, _giveawayData) {
    ...
    const messageCountInput = new discord_js_1.TextInputBuilder()
        ...
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setRequired(false);
    modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(rolesInput), new discord_js_1.ActionRowBuilder().addComponents(accountAgeInput), new discord_js_1.ActionRowBuilder().addComponents(messageCountInput));
    await interaction.showModal(modal);
}
async function handleGiveawayCreation(interaction, giveawayData, _client) {
    // Validate all required fields
    if (!giveawayData.prize || !giveawayData.duration || !giveawayData.channelId) {
        await interaction.reply({
            embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Please complete all required fields before creating the giveaway.")],
            flags: "Ephemeral",
        });
        return;
    }
    try {
        await main_1.GiveawayManager.waitUntilReady(); // Espera a que el manager esté listo
        const channel = await interaction.guild?.channels.fetch(giveawayData.channelId);
        if (!channel?.isTextBased()) {
            await interaction.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("The selected channel is no longer available.")],
                flags: "Ephemeral",
            });
            return;
        }
        const giveawayOptions = {
            ...
            hostedBy: interaction.user, // Pass the User object instead of the ID
            messages: {
                giveaway: "🎉 **GIVEAWAY EVENT STARTED!** 🎉\n\nDon't miss your chance to win an exclusive prize! React with 🎉 below to participate.",
                ...
                winMessage: "🎊 Congratulations {winners}! You have won **{prize}**! Please contact the staff to claim your reward.",
        ...
        // Start the giveaway
        const giveaway = await main_1.GiveawayManager.startGiveaway(channel, giveawayOptions);
        ...
        // Save to database
        await main_1.main.prisma.giveaway.create({ data: dbData });
        await interaction.reply({
            embeds: [
        ...
        await interaction.reply({
            embeds: [new embeds_extend_1.ErrorEmbed().setDescription("An error occurred while creating the giveaway. Please try again.")],
            flags: "Ephemeral",
        });
    }
}
async function processPrizeModal(interaction, giveawayData) {
    ...
    if (!prize) {
        await interaction.reply({
            embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Please provide a valid prize description.")],
            flags: "Ephemeral",
        });
        return;
    }
    ...
    await interaction.reply({
        embeds: [new embeds_extend_1.EmbedCorrect().setDescription(`**Prize set:** ${prize}\n` + `**Winners:** ${winners}`)],
        flags: "Ephemeral",
    });
}
async function processDurationModal(interaction, giveawayData) {
    const durationInput = interaction.fields.getTextInputValue("duration_input").trim();
    const duration = parseDuration(durationInput);
    if (!duration) {
        await interaction.reply({
            embeds: [
                new embeds_extend_1.ErrorEmbed().setDescription("Invalid duration format. Please use formats like:\n" +
                    "- `1d` (1 day)\n" +
                    "- `12h` (12 hours)\n" +
                    "- `30m` (30 minutes)\n" +
    ...
    if (duration < minDuration) {
        await interaction.reply({
            embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Giveaway duration must be at least 5 minutes.")],
            flags: "Ephemeral",
        });
        return;
    }
    ...
    if (duration > maxDuration) {
        await interaction.reply({
            embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Giveaway duration cannot exceed 30 days.")],
            flags: "Ephemeral",
        });
        return;
    }
    ...
    await interaction.reply({
        embeds: [new embeds_extend_1.EmbedCorrect().setDescription(`**Duration set:** ${formatDuration(duration)}`)],
        flags: "Ephemeral",
    });
}
async function processRequirementsModal(interaction, giveawayData) {
    ...
    if (Object.keys(requirements).length > 0) {
        ...
        await interaction.reply({
            embeds: [
                new embeds_extend_1.EmbedCorrect().setDescription("**Requirements set:**\n" +
        ...
        await interaction.reply({
            embeds: [new embeds_extend_1.EmbedCorrect().setDescription("No requirements set - giveaway will be open to everyone.")],
            flags: "Ephemeral",
        });
    }
}
...
module.exports = GiveawayCreate;
//# sourceMappingURL=create.js.map
//# debugId=8f09c911-65ef-59b1-a0c2-67a4aebab182
