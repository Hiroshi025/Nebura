"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="d9ddd9dc-23cc-58e2-aa3d-a2e3625c1a5f")}catch(e){}}();
    ...
    async execute(_client, message, args) {
        if (!message.guild) {
            return message.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("This command can only be used in a server.")],
        ...
        try {
            ...
            const activeGiveaways = await main_1.GiveawayManager.getManager()
                .giveaways.filter((g) => g.guildId === message.guild?.id && !g.ended)
                .sort((a, b) => a.endAt - b.endAt); // Sort by soonest to end
            if (activeGiveaways.length === 0) {
                return message.reply({
                    embeds: [new embeds_extend_1.ErrorEmbed().setDescription("There are no active giveaways to end in this server.")],
                    allowedMentions: { repliedUser: false },
                });
            }
            ...
            const selectMenu = new discord_js_1.StringSelectMenuBuilder()
                ...
                .addOptions(activeGiveaways.slice(0, 25).map((g) => ({
            ...
            const embed = new embeds_extend_1.ErrorEmbed()
            ...
            collector.on("collect", async (selectInteraction) => {
                ...
                if (!selectedGiveaway) {
                    await selectInteraction.reply({
                        embeds: [new embeds_extend_1.ErrorEmbed().setDescription("The selected giveaway could not be found.")],
                        ephemeral: true,
                    });
                    return;
                }
            ...
            await message.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("An error occurred while processing the command.")],
                allowedMentions: { repliedUser: false },
            });
        }
    },
 ...
 */
async function endSpecificGiveaway(message, giveawayId) {
    try {
        const giveaway = await main_1.GiveawayManager.getManager().giveaways.find((g) => g.messageId === giveawayId && g.guildId === message.guild?.id);
        if (!giveaway) {
            return message.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription(`No active giveaway found with ID \`${giveawayId}\` in this server.`)],
                allowedMentions: { repliedUser: false },
            });
        }
        if (giveaway.ended) {
            return message.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription(`The giveaway for **${giveaway.prize}** has already ended.`)],
                allowedMentions: { repliedUser: false },
            });
        }
        ...
        await message.reply({
            embeds: [new embeds_extend_1.ErrorEmbed().setDescription("Failed to process the specified giveaway.")],
            allowedMentions: { repliedUser: false },
        });
    }
}
 ...
 */
async function confirmGiveawayEnd(interaction, giveaway) {
    try {
        const isMessage = interaction instanceof discord_js_1.Message;
        const userId = isMessage ? interaction.author.id : interaction.user.id;
        const embed = new embeds_extend_1.ErrorEmbed()
            .setTitle("⚠️ Confirm Giveaway End")
            .setDescription(`You are about to end the giveaway for **${giveaway.prize}**\n\n` +
        ...
        collector.on("collect", async (buttonInteraction) => {
            try {
                if (buttonInteraction.customId === `giveaway_end_confirm_${giveaway.messageId}`) {
                    ...
                    // End the giveaway
                    await main_1.GiveawayManager.getManager().end(giveaway.messageId);
                    await buttonInteraction.followUp({
                        embeds: [
                            new embeds_extend_1.EmbedCorrect().setDescription(`Successfully ended the giveaway for **${giveaway.prize}**!\n` +
                    ...
                    await buttonInteraction.update({
                        embeds: [new embeds_extend_1.EmbedCorrect().setDescription("Giveaway ending cancelled.")],
                        components: [],
                    });
                }
            }
                ...
                if (buttonInteraction.replied || buttonInteraction.deferred) {
                    await buttonInteraction.followUp({
                        embeds: [
                            new embeds_extend_1.ErrorEmbed().setDescription("Failed to end the giveaway. Please try again or check permissions."),
                        ],
                        ephemeral: true,
                    });
                }
        ...
        if (interaction instanceof discord_js_1.Message) {
            await interaction.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("An error occurred during confirmation.")],
                allowedMentions: { repliedUser: false },
            });
        }
        else {
...
function formatDuration(remainingTime) {
    ...
    if (seconds > 0 && parts.length === 0)
        parts.push(`${seconds}s`); // Only show seconds if < 1m left
    return parts.join(" ");
}
module.exports = GiveawayEnd;
//# sourceMappingURL=end.js.map
//# debugId=d9ddd9dc-23cc-58e2-aa3d-a2e3625c1a5f
