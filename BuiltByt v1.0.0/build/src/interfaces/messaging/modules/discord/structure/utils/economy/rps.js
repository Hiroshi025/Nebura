"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="f0451600-57ab-5d84-b296-6b86195be2d0")}catch(e){}}();
...
async function RPSCommand(interaction, _client) {
    ...
    if (!bet || bet < 200) {
        if (!interaction.replied) {
            return interaction.reply({
                embeds: [new embeds_extend_1.ErrorEmbed().setDescription("The minimum bet for Rock, Paper, Scissors is $200.")],
                flags: "Ephemeral",
            });
        }
        return;
    ...
    if (bet > challengerBalance.balance) {
        if (!interaction.replied) {
            return interaction.reply({
                embeds: [
                    new embeds_extend_1.ErrorEmbed().setDescription(`You don't have enough balance to place this bet! Your current balance is $${challengerBalance.balance}.`),
                ],
                flags: "Ephemeral",
            });
        }
    ...
    if (opponent) {
        if (opponent.bot || opponent.id === challenger.id) {
            if (!interaction.replied) {
                return interaction.reply({
                    embeds: [new embeds_extend_1.ErrorEmbed().setDescription("You must mention a valid user to challenge.")],
                    flags: "Ephemeral",
                });
            }
            return;
        ...
        if (bet > opponentBalance.balance) {
            if (!interaction.replied) {
                return interaction.reply({
                    embeds: [
                        new embeds_extend_1.ErrorEmbed().setDescription(`The opponent doesn't have enough balance to accept this bet! Their current balance is $${opponentBalance.balance}.`),
                    ],
                    flags: "Ephemeral",
                });
            }
        ...
        const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId("accept_rps").setLabel("Accept").setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId("reject_rps").setLabel("Reject").setStyle(discord_js_1.ButtonStyle.Danger));
...
async function startGame(interaction, challenger, opponent, bet) {
    ...
    const collector = gameMessage.createMessageComponentCollector({
        filter: (i) => players.some((p) => p.id === i.user.id) && ["rock", "paper", "scissors", "forfeit"].includes(i.customId),
        time: 30000,
    });
    collector.on("collect", async (i) => {
...
//# debugId=f0451600-57ab-5d84-b296-6b86195be2d0
