"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="dfb3e57b-8c7a-5451-b803-a12a1417ef98")}catch(e){}}();
...
class GiveawayService {
     ...
     */
    constructor() {
        this.manager = new discord_giveaways_1.GiveawaysManager(main_1.client, {
            default: {
                botsCanWin: false,
    ...
    async loadGiveaways() {
        try {
            const giveaways = await main_1.main.prisma.giveaway.findMany({
            ...
            const loadPromises = giveaways.map(async (giveaway) => {
                ...
                if (!channel) {
                    console.warn(`[GiveawayService] Channel ${giveaway.channelId} not found or invalid for giveaway ${giveaway.messageId}`);
                    return;
                }
                // Fix: Ensure createdTimestamp is not null for ThreadChannels
                if ("createdTimestamp" in channel && channel.createdTimestamp === null) {
                    console.warn(`[GiveawayService] Channel ${giveaway.channelId} has null createdTimestamp and cannot be used for giveaway ${giveaway.messageId}`);
                    return;
                }
                const options = {
                    ...
                    hostedBy: main_1.client.users.cache.get(giveaway.hostedBy),
                    messages: {
                        giveaway: "🎉 **GIVEAWAY** 🎉",
                ...
                try {
                    await this.manager.start(channel, options);
     ...
     */
    async getValidChannel(channelId) {
        try {
            const channel = await main_1.client.channels.fetch(channelId);
            ...
            if ("permissionsFor" in channel && typeof channel.permissionsFor === "function") {
                const permissions = channel.permissionsFor(main_1.client.user.id);
    ...
    setupListeners() {
        ...
        this.manager.on("giveawayEnded", async (giveaway, winners) => {
            try {
                ...
                const giveawayData = await main_1.main.prisma.giveaway.findUnique({
                ...
                if (giveawayData.requirements) {
                    deleteOperations.push(main_1.main.prisma.giveawayRequirements.delete({
                        where: { giveawayId: giveawayData.id },
                    }));
                }
                // Delete the main giveaway entry
                deleteOperations.push(main_1.main.prisma.giveaway.delete({
                    where: { messageId: giveaway.messageId },
                }));
                // 3. Execute transaction
                await main_1.main.prisma.$transaction(deleteOperations);
                // 4. Log successful deletion
                console.log(`[GiveawayService] Successfully deleted giveaway ${giveaway.messageId} and its requirements from DB`);
                ...
                try {
                    await main_1.main.prisma.giveaway.deleteMany({
                        where: { messageId: giveaway.messageId },
                    });
                    ...
                    await channel.send({
                        embeds: [
                            new embeds_extend_1.EmbedCorrect()
                                ...
                                .setDescription(`The giveaway for **${giveaway.prize}** has been rerolled!\n` +
        ...
        this.manager.on("giveawayReactionAdded", async (giveaway, member) => {
            try {
                ...
                const dbGiveaway = await main_1.main.prisma.giveaway.findUnique({
                    where: { messageId: giveaway.messageId },
                    include: { requirements: true },
                });
                const requirements = dbGiveaway?.requirements;
                if (requirements) {
                            ...
                            await member.send({
                                embeds: [
                                    new embeds_extend_1.ErrorEmbed()
                                        ...
                                        .setDescription(`You don't have the required roles to enter the giveaway for **${giveaway.prize}**`),
                                ],
                            });
                            try {
                                const channel = await this.getValidChannel(giveaway.channelId);
                                ...
                                const reaction = message.reactions.cache.find((r) => r.emoji.name === (giveaway.options?.reaction || "🎉"));
                                if (reaction) {
                        ...
                        if (accountAgeDays < requirements.minAccountAge) {
                            await member.send({
                                embeds: [
                                    new embeds_extend_1.ErrorEmbed()
                                        .setTitle("Entry Requirements Not Met")
                                        .setDescription(`Your account must be at least ${requirements.minAccountAge} days old to enter this giveaway`),
                                ],
                            });
                            try {
                                const channel = await this.getValidChannel(giveaway.channelId);
                        ...
                        if (userMessages < requirements.minMessages) {
                            await member.send({
                                embeds: [
                                    new embeds_extend_1.ErrorEmbed()
                                        .setTitle("Entry Requirements Not Met")
                                        .setDescription(`You need at least ${requirements.minMessages} messages in this server to enter this giveaway`),
                                ],
                            });
                ...
                await member.send({
                    embeds: [
                        new embeds_extend_1.ErrorEmbed()
                            ...
                            .setDescription(`The giveaway for **${giveaway.prize}** has already ended. ` +
                ...
                await member.send({
                    embeds: [
                        new embeds_extend_1.EmbedCorrect()
                            ...
                            .setDescription(`You've been removed from the giveaway for **${giveaway.prize}**. ` +
        ...
        this.manager.on("giveawayDeleted", async (giveaway) => {
            try {
                ...
                await main_1.main.prisma.giveaway.delete({
                    where: { messageId: giveaway.messageId },
                });
                    ...
                    await channel.send({
                        embeds: [
                            new embeds_extend_1.ErrorEmbed()
                                ...
                                .setDescription(`The giveaway for **${giveaway.prize}** has been cancelled. ` + "All entries have been voided."),
                        ],
                    });
                }
            }
     ...
     */
    async startGiveaway(channel, options) {
        try {
            ...
            const dbData = {
                ...
                hostedBy: options.hostedBy?.id || main_1.client.user.id,
                ...(options.requirements && {
                    requirements: {
                        create: {
     ...
     */
    async rerollGiveaway(messageId, options) {
        try {
...
//# debugId=dfb3e57b-8c7a-5451-b803-a12a1417ef98
