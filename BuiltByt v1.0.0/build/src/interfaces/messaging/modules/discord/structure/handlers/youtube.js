"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="a1942e2b-45b9-5fcf-9072-b9fd87000ef4")}catch(e){}}();
 ...
 */
const YouTube = async (client) => {
    console.info(`[YOUTUBE][INIT] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Initializing YouTube notification system for Discord.`);
     ...
     */
    client.Youtubelog = new cron_1.CronJob("0 1,9,17,23,29,35,41,47,53,59 * * * *", async function () {
        // Log the start of the cron job
        console.debug(`[YOUTUBE][CRON][START] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Checking YouTube accounts.`);
        // Fetch all guilds (Discord servers) with YouTube configuration from the database
        let guilds = await main_1.main.prisma.youtube.findMany({
            include: {
                youtubers: true,
            },
        });
        console.debug(`[YOUTUBE][CRON][DB] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Found ${guilds.length} guilds with YouTube configuration.`);
        if (!guilds) {
            // No guilds found in the database
            console.warn(`[YOUTUBE][CRON][DB] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: No guilds found in database.`);
            return;
        }
        ...
        for await (const g of guilds) {
            ...
            if (!guild) {
                ...
                console.warn(`[YOUTUBE][CRON][GUILD] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Guild not found in cache: ${g.serverId}`);
                continue;
            }
            console.debug(`[YOUTUBE][CRON][GUILD] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Processing guild: ${guild.name} (${guild.id})`);
            // Wait 2 seconds before checking videos for each guild to avoid rate limits
            await setTimeout(() => getVideos(guild), (0, ms_1.default)("2s"));
        }
        // Log the end of the cron job
        console.debug(`[YOUTUBE][CRON][END] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Finished checking all guilds.`);
    }, null, true, "Europe/Berlin");
    // Start the cron job when the Discord client is ready
    client.on("ready", async () => {
        console.info(`[YOUTUBE][READY] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: YouTube system started. Starting cron job.`);
     ...
     */
    async function getVideos(guild) {
        console.debug(`[YOUTUBE][GUILD][START] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Checking YouTube channels for guild: ${guild.name} (${guild.id})`);
        // Fetch all YouTube channel configurations for this guild from the database
        let tempData = await main_1.main.prisma.youtuber.findMany({ where: { guildId: guild.id } });
        console.debug(`[YOUTUBE][GUILD][DB] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Found ${tempData.length} YouTube channels for guild: ${guild.name} (${guild.id})`);
        if (!tempData || !tempData.length) {
            // No YouTube channels configured for this guild
            console.warn(`[YOUTUBE][GUILD][DB] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: No YouTube channels configured for guild: ${guild.name} (${guild.id})`);
            return;
        }
        // Iterate over each YouTube channel configuration for this guild
        for (const chan of tempData) {
            console.debug(`[YOUTUBE][CHANNEL][START] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Processing channel config: ${JSON.stringify(chan)}`);
            // Validate channel configuration
            if (!chan.userId || !chan.channelId) {
                // Missing required fields
                console.warn(`[YOUTUBE][CHANNEL][SKIP] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Missing userId or channelId for config: ${JSON.stringify(chan)}`);
                continue;
            }
            if (chan.channelId === undefined || chan.channelId.length < 18) {
                // Invalid Discord channel ID
                console.warn(`[YOUTUBE][CHANNEL][SKIP] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Invalid channelId: ${chan.channelId}`);
                continue;
            }
            try {
                ...
                if (!channel) {
                    ...
                    console.error(`[YOUTUBE][CHANNEL][INFO_ERROR] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} | Channel: ${chan.userId} | Error: Channel not found`);
                    continue;
                }
                ...
                if (!videos || videos.length === 0) {
                    ...
                    console.warn(`[YOUTUBE][CHANNEL][NO_VIDEO] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: No videos found for channel: ${chan.userId}`);
                    continue;
                }
                ...
                let title = latestVideo.title?.text ||
                    (Array.isArray(latestVideo.title?.runs) ? latestVideo.title.runs.map((r) => r.text).join("") : "\u200b");
                // Extraer descripción como string
                let description = latestVideo.description_snippet?.text ||
                    (Array.isArray(latestVideo.description_snippet?.runs)
                        ? latestVideo.description_snippet.runs.map((r) => r.text).join("")
                        : "\u200b");
                // Extraer duración
                ...
                let embed = new embeds_extend_1.EmbedCorrect()
                    .setAuthor({
                    name: `${authorName}`,
                ...
                try {
                    ...
                    if (chan.lastVideo !== lastVideoId) {
                        ...
                        await ch
                            ...
                            console.info(`[YOUTUBE][NOTIFY][SUCCESS] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} | Guild: ${guild.name} (${guild.id}) | Channel: ${chan.channelId} | Video: ${lastVideoUrl} | Notification sent successfully.`);
                        })
                            .catch((e) => {
                            // Handle errors sending the notification
                            console.error(`[YOUTUBE][NOTIFY][ERROR] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} | Guild: ${guild.name} (${guild.id}) | Channel: ${chan.channelId} | Error sending notification:`, e);
                        });
                    }
                    else {
                        // No new video to notify
                        console.debug(`[YOUTUBE][NOTIFY][SKIP] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} | Guild: ${guild.name} (${guild.id}) | Channel: ${chan.channelId} | No new video to notify.`);
                    }
                }
                catch (e) {
                    // Handle errors fetching the Discord channel
                    console.error(`[YOUTUBE][CHANNEL][FETCH_ERROR] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} | Guild: ${guild.name} (${guild.id}) | Channel: ${chan.channelId} | Error fetching Discord channel:`, e);
                }
            }
            catch (error) {
                // Handle errors fetching YouTube channel or video data
                console.error(`[YOUTUBE][CHANNEL][ERROR] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} | Channel: ${chan.userId} | Error:`, error);
            }
            console.debug(`[YOUTUBE][CHANNEL][END] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Finished processing channel config: ${JSON.stringify(chan)}`);
        }
        console.debug(`[YOUTUBE][GUILD][END] | ${(0, moment_1.default)().format("YYYY-MM-DD HH:mm:ss")} :: Finished checking YouTube channels for guild: ${guild.name} (${guild.id})`);
    }
};
exports.YouTube = YouTube;
//# sourceMappingURL=youtube.js.map
//# debugId=a1942e2b-45b9-5fcf-9072-b9fd87000ef4
