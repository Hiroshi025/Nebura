"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="f157b590-2a6f-5243-9d63-f2d2e6f2e733")}catch(e){}}();
...
async function Ranking(message, client) {
    ...
    if (cooldown.has(userId)) {
        (0, console_1.logWithLabel)("info", `User: ${message.author.tag} | Cooldown activated.`, {
            customLabel: "Rank",
        });
        return;
    }
    ...
    let rankingConfig = await main_1.main.prisma.levelConfig.findFirst({
        where: { guildId, status: true },
    });
    // Create default config if none exists
    if (!rankingConfig) {
        rankingConfig = await main_1.main.prisma.levelConfig.create({
            data: { guildId, channelId: null, status: true },
        });
    }
    // Skip if ranking is disabled
    if (!rankingConfig.status) {
        (0, console_1.logWithLabel)("info", `Ranking is disabled for guild: ${guildId}`, {
            customLabel: "Rank",
        });
        return;
    }
    ...
    const streakBonus = streakUpdate.currentStreak > 1
    ...
    // Log activity
    (0, console_1.logWithLabel)("info", `User: ${message.author.tag} | XP: ${xp}/${level * 100} | Level: ${level} | Streak: ${streakUpdate.currentStreak} | Earned: ${totalXp} XP (Base: ${xpAmount}, Streak: ${streakBonus}, Quests: ${questXp})`, {
        customLabel: "Rank",
    });
    // Check for level up
...
// 2. Streak system
async function updateStreak(userId, guildId, currentDay) {
    // First, try to find the existing streak
    const existingStreak = await main_1.main.prisma.streak.findUnique({
    ...
    if (existingStreak) {
        ...
        const updatedStreak = await main_1.main.prisma.streak.update({
            where: { userId_guildId: { userId, guildId } },
            data: {
                lastActive: currentDay,
        ...
        const createdStreak = await main_1.main.prisma.streak.create({
            data: {
                userId,
                guildId,
                lastActive: currentDay,
...
async function checkLeaderboardReset(guildId, currentWeek, currentMonth) {
    const lastReset = await main_1.main.prisma.leaderboardReset.findFirst({ where: { guildId } });
    if (!lastReset || lastReset.lastWeek !== currentWeek) {
        // Weekly reset
        await main_1.main.prisma.$transaction([
            main_1.main.prisma.weeklyLeaderboard.create({
                data: {
                    guildId,
            ...
            main_1.main.prisma.userLevel.updateMany({
                where: { guildId },
                data: { weeklyXp: 0 },
            }),
            main_1.main.prisma.leaderboardReset.upsert({
                where: { guildId },
    ...
    if (!lastReset || lastReset.lastMonth !== currentMonth) {
        // Monthly reset
        await main_1.main.prisma.$transaction([
            main_1.main.prisma.monthlyLeaderboard.create({
                data: {
                    guildId,
...
// 4. Quest system
async function updateQuests(userId, guildId, message) {
    const quests = await main_1.main.prisma.userQuests.findUnique({
        where: { userId_guildId: { userId, guildId } },
    });
    if (!quests)
        return { completedQuests: [], updatedQuests: quests };
    ...
    // Check daily quests
    for (const quest of constant_1.DAILY_QUESTS) {
        ...
        if (quest.check(message, dailyProgress[quest.id] || 0)) {
            ...
            if (newProgress >= quest.requirement && !dailyCompleted[quest.id]) {
                ...
                updatedQuests.dailyCompleted[quest.id] = true;
            }
            updatedQuests.dailyProgress[quest.id] = newProgress;
        }
    }
    // Check weekly quests (similar logic)
    // Save updated quests
    await main_1.main.prisma.userQuests.update({
        where: { userId_guildId: { userId, guildId } },
        data: {
...
// 5. Channel bonus
async function getChannelBonus(channelId, rankingConfig) {
    if (!rankingConfig.bonusChannels)
        return null;
    let bonusChannels = [];
    ...
    return bonusChannels.find((b) => b.channelId === channelId);
}
// 6. Prestige system
async function handlePrestige(user, message, _client) {
    ...
    const buffer = await (0, discord_arts_1.Profile)(user.userId, {
    ...
    const attachment = new discord_js_1.AttachmentBuilder(buffer, { name: "prestige.png" });
    try {
...
// 7. Anti-spam system
async function handleSpamPenalty(message, _client) {
    const guildId = message.guild?.id;
    const userId = message.author.id;
    if (!guildId)
        return;
    ...
    await main_1.main.prisma.userLevel.update({
        where: { guildId_userId: { guildId, userId } },
        data: { xp: { decrement: constant_1.SPAM_PENALTY_XP } },
    });
    // Log and notify
    (0, console_1.logWithLabel)("warning", `Spam detected from ${message.author.tag}, applying XP penalty`, {
        customLabel: "Ranking",
        context: { userId, userName: message.author.username },
    });
    try {
        await message.author.send(`⚠️ Please avoid spamming! ${constant_1.SPAM_PENALTY_XP} XP has been deducted.`);
    }
    catch (err) {
...
async function checkAchievements(userId, guildId, level, messageCount) {
    const unlockedAchievements = [];
    const userAchievements = await main_1.main.prisma.userAchievements.findUnique({
        where: { userId_guildId: { userId, guildId } },
    });
    // Check level-based achievements
    for (const achievement of constant_1.ACHIEVEMENTS) {
        if (!userAchievements?.achievements.includes(achievement.id)) {
            if ((achievement.type === "LEVEL" && level >= achievement.requirement) ||
    ...
    if (unlockedAchievements.length > 0) {
        await main_1.main.prisma.userAchievements.upsert({
            where: { userId_guildId: { userId, guildId } },
            update: {
...
function isYesterday(lastDateStr, currentDateStr) {
 ...
 */
async function sendLevelUpNotification(message, client, rankingConfig, details) {
    ...
    const buffer = await (0, discord_arts_1.Profile)(userId, {
    ...
    const attachment = new discord_js_1.AttachmentBuilder(buffer, { name: "levelup.png" });
    try {
 ...
 */
async function getTopUsers(guildId, xpField, limit = 10) {
    return await main_1.main.prisma.userLevel.findMany({
        where: { guildId },
 ...
 */
async function awardTopUsers(guildId, period) {
    ...
    const config = await main_1.main.prisma.levelConfig.findUnique({
        where: { guildId },
    ...
    for (const user of topUsers) {
        ...
        if (reward.roleId) {
            try {
                const guild = await main_1.client.guilds.fetch(guildId);
    ...
    // Log the rewards
    (0, console_1.logWithLabel)("info", `Awarded ${period} top users in guild ${guildId}`, {
        customLabel: "Rank",
    });
}
//# sourceMappingURL=helpers.js.map
//# debugId=f157b590-2a6f-5243-9d63-f2d2e6f2e733
