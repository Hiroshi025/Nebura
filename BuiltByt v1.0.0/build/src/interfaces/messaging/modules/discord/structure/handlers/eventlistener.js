"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="7170ff29-f784-50e7-9244-d9674fbbac91")}catch(e){}}();
                ...
                switch (event) {
                    case "VoiceStateUpdate":
                        this.client.on(discord_js_1.Events.VoiceStateUpdate, async (oldState, newState) => {
                            ...
                            if (oldState.guild.id !== guild.id)
                                return; // Ensure the event is for the current guild
                            if (oldState.channelId === newState.channelId)
                                return; // Ignore if the channel hasn't changed
                            if (oldState.selfMute !== newState.selfMute || oldState.selfDeaf !== newState.selfDeaf) {
                                setTimeout(async () => {
                                    const fields = [
                                        {
                                            ...
                                            value: [
                                                `> **User:** ${(0, discord_js_1.userMention)(newState.id)}`,
                                    ...
                                    this.send_log(guild, newState.selfMute || newState.selfDeaf ? "Red" : "Green", "Events Logger - Voice State Update", [
                                        `> **User ID:** \`${newState.id}\``,
                            ...
                            if (newState.channelId && !oldState.channelId) {
                                setTimeout(async () => {
                                    ...
                                    this.send_log(guild, "Green", "Events Logger - Voice Channel Join", [`> **User ID:** \`${newState.id}\``, `> **Channel ID:** \`${newState.channelId}\``].join("\n"), guild.iconURL({ forceStatic: true }), fields);
                                }, this.delay);
                            }
                        });
                        break;
                    case "InviteDelete":
                        this.client.on(discord_js_1.Events.InviteDelete, async (invite) => {
                                ...
                                const fields = [
                                    {
                                        ...
                                        value: [
                                            ...
                                            `> **Invite Inviter:** ${(0, discord_js_1.userMention)(invite.inviter?.id ?? "Unknown")}`,
                                        ].join("\n"),
                                        inline: false,
                                    },
                                ];
                                this.send_log(guild, "Red", "Events Logger - Invite Delete", [
                    ...
                    case "InviteCreate":
                        this.client.on(discord_js_1.Events.InviteCreate, async (invite) => {
                            ...
                            setTimeout(async () => {
                                ...
                                this.send_log(guild, "Green", "Events Logger - Invite Create", [
                                    `> **Invite Code:** \`${invite.code}\``,
                                    `> **Invite Uses:** ${invite.uses}`,
                                    `> **Invite Max Uses:** ${invite.maxUses ?? "Unlimited"}`,
                                    `> **Invite Max Age:** ${invite.maxAge ? `${invite.maxAge} seconds` : "No Limit"}`,
                    ...
                    case "AutoModerationRuleCreate":
                        this.client.on(discord_js_1.Events.AutoModerationRuleCreate, async (rule) => {
                            ...
                            this.send_log(guild, "Green", "Events Logger - Auto Moderation Rule Create", [`> **Rule Name:** ${rule.name} (\`${rule.id}\`)`, `> **Rule Type:** ${rule.eventType}`].join("\n"), guild.iconURL({ forceStatic: true }), fields);
                        });
                        break;
                    case "AutoModerationRuleDelete":
                        this.client.on(discord_js_1.Events.AutoModerationRuleDelete, async (rule) => {
                            ...
                            this.send_log(guild, "Red", "Events Logger - Auto Moderation Rule Delete", [`> **Rule Name:** ${rule.name} (\`${rule.id}\`)`, `> **Rule Type:** ${rule.eventType}`].join("\n"), guild.iconURL({ forceStatic: true }), fields);
                        });
                        break;
...
//# debugId=7170ff29-f784-50e7-9244-d9674fbbac91
