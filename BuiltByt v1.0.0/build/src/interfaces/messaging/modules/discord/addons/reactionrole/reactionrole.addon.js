"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="c7bb7599-9100-567a-a1f5-844bda9d8582")}catch(e){}}();
...
exports.default = new addons_1.Addons({
    ...
    client.on(discord_js_1.Events.MessageReactionAdd, async (reaction, user) => {
        try {
            ...
            let reactionSetup = await main_1.main.prisma.reactionRole.findMany({
                where: { guildId },
            });
                                ...
                                if (guildRole) {
                                    ...
                                    if (botHighestRole && botHighestRole.rawPosition > guildRole.rawPosition) {
                                        if (!member.roles.cache.has(param.Role)) {
                                            console.log(`[DEBUG] Adding role ${guildRole.name} to member ${member.user.tag}`);
                            ...
                            try {
                                if (param.Role !== currentRole) {
                                    ...
                                    if (guildRole) {
                                        ...
                                        if (botHighestRole && botHighestRole.rawPosition > guildRole.rawPosition) {
                                            if (member.roles.cache.has(param.Role)) {
                                                console.log(`[DEBUG] Removing role ${guildRole.name} from member ${member.user.tag}`);
    ...
    client.on(discord_js_1.Events.MessageReactionRemove, async (reaction, user) => {
        try {
            ...
            if (!reaction.message.guild ||
                ...
                reaction.message.channel.type !== discord_js_1.ChannelType.GuildText)
                return;
            console.log(`[DEBUG] Getting reaction roles for guild ${reaction.message.guild.id}`);
            const guildId = reaction.message.guild.id;
            const reactionSetup = await main_1.main.prisma.reactionRole.findMany({
                where: { guildId },
            });
            for (const setup of reactionSetup) {
                if (reaction.message.id === setup.messageId) {
                    console.log(`[DEBUG] Found matching reaction role setup for message ${reaction.message.id}`);
            ...
            console.error("[ERROR] In MessageReactionRemove:", e instanceof Error ? e.stack : String(e));
        }
    });
});
//# sourceMappingURL=reactionrole.addon.js.map
//# debugId=c7bb7599-9100-567a-a1f5-844bda9d8582
