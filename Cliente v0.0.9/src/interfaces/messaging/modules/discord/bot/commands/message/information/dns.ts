...
async function showMainMenu(message: any, prefix: string) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("full_analysis")
      .setLabel("Start Full Analysis")
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId("dns_examples")
...
async function showQuickLookup(message: any, domain: string) {
  try {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`dns_full_${domain}`)
        .setLabel("Full Analysis")
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId(`dns_ping_${domain}`)
        .setLabel("Detailed Ping")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
  ...
  try {
    ...
    for (const [type, values] of Object.entries(records)) {
      if (Array.isArray(values) && values.length > 0) {
        ...
        if (type === "MX") {
          valueStr = (values as any[])
            .map((r: any) => `${r.exchange} (priority ${r.priority})`)
            .join("\n");
        } else if (type === "SOA") {
