...
export default ({ app }: TRoutesInput) => {
   ...
   */
  app.post(
    formatRoute("tickets/:userId/:ticketId/message"),
  ...
  app.get(formatRoute("transcripts/:transcriptId"), async (req: Request, res: Response) => {
    ...
    if (
      ...
      Object.prototype.hasOwnProperty.call(
        transcript.content[transcript.content.length - 1],
        "timestamp",
      )
    ) {
      const first = (transcript.content[0] as { timestamp: string | number }).timestamp;
      const last = (
        transcript.content[transcript.content.length - 1] as { timestamp: string | number }
      ).timestamp;
      if (first && last) {
   ...
   */
  app.get(
    formatRoute("transcripts/:transcriptId/download"),
    async (req: Request, res: Response) => {
      const { transcriptId } = req.params;
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="transcripcion-${transcriptId}.pdf"`,
      );
      ...
      if (Array.isArray(transcript.content)) {
        transcript.content.forEach((msg: any) => {
          doc
            .fontSize(10)
            .text(`[${msg.timestamp}] ${msg.senderName || "Sistema"}: ${msg.content}`);
          doc.moveDown(0.5);
        });
      }
   ...
   */
  app.get(
    formatRoute("tickets/:userId/:ticketId/messages"),
    async (req: Request, res: Response) => {
      const { userId, ticketId } = req.params;
      if (!userId || !ticketId) {
        return res.status(400).json({
