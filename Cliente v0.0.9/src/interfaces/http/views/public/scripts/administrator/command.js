document.addEventListener("DOMContentLoaded", function () {
  ...
  function initCommandManagement() {
    ...
    const embedFieldsInput = document.getElementById("commandEmbedFields");
    if (embedFieldsInput) embedFieldsInput.addEventListener("input", updateEmbedPreview);
  ...
  function showCommandModal(command = null) {
    ...
    if (command) {
      ...
      // Manejo seguro de fields para evitar error de JSON.parse
      let fieldsValue = "";
      if (command.fields) {
        try {
          if (typeof command.fields === "string") {
            // Si es string, intenta parsear
            fieldsValue = JSON.stringify(JSON.parse(command.fields), null, 2);
          } else if (Array.isArray(command.fields) || typeof command.fields === "object") {
            // Si ya es objeto/array, solo serializa
            fieldsValue = JSON.stringify(command.fields, null, 2);
          }
        } catch (e) {
          // Si falla el parseo, muestra como string plano
          fieldsValue = String(command.fields);
        }
      }
      document.getElementById("commandEmbedFields").value = fieldsValue;
      ...
      form.reset();
      document.getElementById("commandId").value = ""; // <-- Limpia el ID
      ...
      document.getElementById("commandEmbedFields").value = "";
  ...
  function updateEmbedPreview() {
    ...
    const embedFieldsInput = document.getElementById("commandEmbedFields").value;
    ...
    // Add fields to preview if they exist and are valid
    if (embedFieldsInput && embedFieldsInput.trim() !== "") {
      try {
        const fields = JSON.parse(embedFieldsInput);
        if (Array.isArray(fields)) {
          fields.forEach((field) => {
            if (field.name && field.value) {
              const fieldHTML = `
                <div class="embed-preview-field ${field.inline ? "embed-preview-field-inline" : ""}">
                  <div class="embed-preview-field-name">${field.name}</div>
                  <div class="embed-preview-field-value">${field.value}</div>
                </div>
              `;
  ...
  function saveCommand() {
      ...
      if (buttonsInput && buttonsInput.trim() !== "") {
        ...
        buttonsValue.forEach((btn) => {
          if (!btn.label || !btn.style) {
            throw new Error("Each button must have label and style properties");
          }
          if (btn.style === "Link") {
            if (!btn.url) throw new Error("Link buttons must have a url property");
          } else {
            if (!btn.customId) throw new Error("Non-link buttons must have a customId property");
          }
        });
      }
    } catch (e) {
    ...
    try {
      const fieldsInput = document.getElementById("commandEmbedFields").value;
      if (fieldsInput && fieldsInput.trim() !== "") {
        fieldsValue = JSON.parse(fieldsInput);
        ...
        fieldsValue.forEach((field) => {
          if (!field.name || !field.value) {
            throw new Error("Each field must have name and value properties");
          }
        });
      }
    } catch (e) {
      showToast("Error", "Invalid fields format: " + e.message, "danger");
      return;
    }
    ...
    const commandData = {
      ...
      embedImage: document.getElementById("commandEmbed").checked
        ...
        : undefined,
      file: document.getElementById("commandFile").value,
      buttons: buttonsValue ? JSON.stringify(buttonsValue) : undefined,
      fields: fieldsValue ? JSON.stringify(fieldsValue) : undefined,
