        ...
        try {
                ...
                return message.reply({
                    embeds: [
                        new embeds_extender_1.ErrorEmbed().setDescription([
                            `${client.getEmoji(message.guild.id, "error")} The \`package name\` is a required argument that is missing.`,
                            `Usage: \`${prefix}npm <package name>\``,
                        ].join("\n")),
                    ],
                });
            const body = await axios_1.default.get(`https://registry.npmjs.com/${pkg}`).then((res) => {
                if (res.status === 404)
            ...
            return message.channel.send({
              ...
              **__Creation & Modification__**
              > **Created at:** <t:${Math.floor(new Date(body.time.created).getTime() / 1000)}:R> | \`${(0, moment_1.default)(body.time.created).fromNow()}\`
              > **Modified at:** \`${new Date(body.time.modified).toDateString()}\` | \`${(0, moment_1.default)(body.time.modified).fromNow()}\`
            ...
            (0, console_1.logWithLabel)("error", `The error has occurred: ${e}`);
            console.error(e);
        }
        return;
    },
