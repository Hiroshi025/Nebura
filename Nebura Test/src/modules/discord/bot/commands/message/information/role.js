...
const roleInfo = {
    ...
    async execute(_client, message, args) {
        ...
        try {
            ...
            if (!roleName)
                return message.reply("Please specify a role name or mention.");
            ...
            const embed = new embeds_extender_1.EmbedCorrect()
                ...
                .addFields({ name: "ID", value: role.id, inline: true }, { name: "Color", value: role.hexColor, inline: true }, { name: "Position", value: role.position.toString(), inline: true }, { name: "Members", value: role.members.size.toString(), inline: true }, {
                name: "Created",
                value: `<t:${Math.floor(role.createdTimestamp / 1000)}:R>`,
                inline: true,
            }, { name: "Hoisted", value: role.hoist ? "Yes" : "No", inline: true }, { name: "Mentionable", value: role.mentionable ? "Yes" : "No", inline: true }, { name: "Managed", value: role.managed ? "Yes" : "No", inline: true }, {
                name: "Permissions",
                value: role.permissions.toArray().length > 0
                    ? (0, discord_js_1.inlineCode)(role.permissions.toArray().join(", "))
                    : "None",
                inline: false,
            });
