    ...
    async execute(client, message, args) {
        ...
        if (!subCommand || !["add", "remove"].includes(subCommand)) {
            return message.reply({
                embeds: [
                    new embeds_extender_1.ErrorEmbed().setDescription("Invalid subcommand. Use `warn add @user reason` or `warn remove warn_id`."),
                ],
            });
        }
        switch (subCommand) {
            case "add": {
                ...
                if (!user) {
                    return message.reply({
                        embeds: [new embeds_extender_1.ErrorEmbed().setDescription("You must mention a user to warn.")],
                    });
                }
            ...
            case "remove": {
                ...
                if (!warnId) {
                    return message.reply({
                        embeds: [new embeds_extender_1.ErrorEmbed().setDescription("You must provide a warn ID to remove.")],
                    });
                }
                const data = await main_1.main.prisma.userWarn.findUnique({ where: { id: warnId } });
                if (!data) {
                    return message.reply({
                        embeds: [new embeds_extender_1.ErrorEmbed().setDescription(`No warn found with ID \`${warnId}\`.`)],
                    });
                }
