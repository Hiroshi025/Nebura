...
exports.default = new builders_1.Event("guildMemberAdd", async (member) => {
    ...
    collector?.on("collect", async (msg) => {
        ...
        if (memberInput === captcha.text?.toLowerCase()) {
            ...
            if (role) {
                ...
                member.send({
                    embeds: [new embeds_extender_1.EmbedCorrect().setDescription(messages.success)],
                });
                if (attempts <= 2) {
                    collector.stop();
                }
                ...
                member.send({
                    embeds: [new embeds_extender_1.EmbedCorrect().setDescription(messages.roleInvalid)],
                });
            }
        }
        else {
            if (attempts >= 3) {
                member.send({
                    embeds: [
                        new embeds_extender_1.EmbedCorrect().setDescription(`${messages.failed} You will be kicked in 1 minute if you do not complete the verification.`),
                    ],
                });
                setTimeout(() => {
    ...
    collector?.on("end", async (_collected, reason) => {
        if (reason === "time") {
            member.kick(messages.failed);
            member.send({
                embeds: [new embeds_extender_1.EmbedCorrect().setDescription(messages.timedout)],
            });
        }
    });
});
