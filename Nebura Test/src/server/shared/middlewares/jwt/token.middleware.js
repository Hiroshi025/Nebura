...
const functions_1 = require("../../../../server/shared/utils/functions");
...
const isAdminToken = (req, res, next) => {
    try {
        const secretAdmin = config_1.config.environments.default["key-secrets"].administrator;
...
const isCustomerToken = (req, res, next) => {
    try {
        const secretCustomer = config_1.config.environments.default["key-secrets"].customer;
        const FORBIDDEN_ERROR = {
            message: "Forbidden: Insufficient privileges",
            timestamp: new Date().toISOString(),
            code: "FORBIDDEN",
