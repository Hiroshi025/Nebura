  ...
  async execute(_client, message) {
    ...
    try {
      ...
      const channelCounts = {
        ...
        announcement: guild.channels.cache.filter((c) => c.type === ChannelType.GuildAnnouncement)
          .size,
      };
      ...
      if (message.content.includes("--txt")) {
        ...
        const attachment = new AttachmentBuilder(Buffer.from(serverData), {
...
function generateServerDataTxt(guild: Guild): string {
  ...
  channelTypes.forEach((type) => {
    ...
    if (channels.size > 0) {
      content += `\n${ChannelType[type]} Channels (${channels.size}):\n`;
      channels.forEach((channel) => {
        content += `- ${channel.name} (${channel.id})`;
        if (channel.parent) content += ` [Category: ${channel.parent.name}]`;
        content += `\n`;
      });
    }
  });
