...
import { MinecraftServerConfig, StatusHandlerConfig } from "@typings/addons";
...
export default new Addons(
  ...
  async () => {
    // Enhanced configuration with multiple servers support
    const configuration: StatusHandlerConfig = {
      enabled: false,
...
/*         {
    ...
    function createStatusEmbed(serverConfig: MinecraftServerConfig, statusData: any): EmbedBuilder {
      ...
      const statusEmoji = isOnline
        ? client.getEmoji(config.project.guildId, "online") || "🟢"
        : client.getEmoji(config.project.guildId, "offline") || "🔴";
    ...
    async function initializeStatusHandler() {
      if (!configuration.enabled) {
        logWithLabel("info", "Minecraft status handler is disabled in configuration");
        return;
      }
