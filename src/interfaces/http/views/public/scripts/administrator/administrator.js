...
// Traduce todos los elementos con data-i18n y data-i18n-placeholder según el idioma actual.
function translatePage() {
  ...
  // Traduce títulos de botones (title)
  document.querySelectorAll("[data-i18n-title]").forEach(el => {
    const key = el.getAttribute("data-i18n-title");
    if (t[key]) el.title = t[key];
  });
  // Traduce opciones de select
...
document.addEventListener("DOMContentLoaded", function () {
  translatePage();
  // Cuando cambie el idioma, vuelve a traducir
  document.querySelectorAll(".lang-link").forEach(link => {
    link.addEventListener("click", () => setTimeout(translatePage, 10));
  });
});
