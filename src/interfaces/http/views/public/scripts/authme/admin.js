document.addEventListener("DOMContentLoaded", function () {
  ...
  const translations = {
    en: {
      ...
      warningMessage:
        "<strong>WARNING:</strong> You are currently in the Admin AuthMe. You have 3 login attempts, otherwise you must wait 1 hour.",
      registerSuccess: "Account created successfully!",
      registerNotAuthorized: "User not found or not authorized.",
      registerServerError: "An unexpected error occurred.",
      loginSuccess: "Login successful!",
      loginInvalid: "Invalid credentials.",
      loginNotAuthorized: "User not authorized.",
      loginServerError: "An unexpected error occurred.",
    },
    es: {
      ...
      warningMessage:
        "<strong>ADVERTENCIA:</strong> Usted se encuentra actualmente en el AuthMe de administración. Tiene 3 intentos para loguearse, en caso contrario deberá esperar 1 hora.",
      registerSuccess: "¡Cuenta creada con éxito!",
      registerNotAuthorized: "Usuario no encontrado o no autorizado.",
      registerServerError: "Ocurrió un error inesperado.",
      loginSuccess: "¡Inicio de sesión exitoso!",
      loginInvalid: "Credenciales inválidas.",
      loginNotAuthorized: "Usuario no autorizado.",
      loginServerError: "Ocurrió un error inesperado.",
    },
  };
  ...
  const setLanguage = (lang) => {
    ...
    // Actualizar la alerta de advertencia
    if (currentTranslations.warningMessage) {
      warningAlert.innerHTML = `<i class="fas fa-exclamation-triangle"></i> ${currentTranslations.warningMessage}`;
    }
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    if (isValid) {
      try {
        const response = await fetch("/dashboard/utils/admin/login", {
          method: "POST",
          headers: {
        ...
        if (response.ok && data.success) {
          ...
          showToast(translations[lang].loginSuccess, translations[lang].loginSuccess, "success");
          setTimeout(() => {
            // agrega el data.user.id al URL de redireccion
            window.location.href = "/dashboard/administrator?id=" + data.user.id;
          }, 1200);
        } else {
          failedAttempts++;
          let toastMsg = translations[lang].loginInvalid;
          let toastType = "danger";
          if (response.status === 403) {
            toastMsg = translations[lang].loginNotAuthorized;
          } else if (response.status === 500) {
            toastMsg = translations[lang].loginServerError;
          }
          if (failedAttempts >= MAX_ATTEMPTS) {
            ...
            showToast(
              "Error",
              "Demasiados intentos fallidos. Su cuenta ha sido bloqueada por 1 hora.",
              "warning",
            );
          } else {
            showToast("Error", toastMsg, toastType);
          }
          showError(
            loginPasswordError,
            `${toastMsg} ${MAX_ATTEMPTS - failedAttempts > 0 ? `Intentos restantes: ${MAX_ATTEMPTS - failedAttempts}` : ""}`,
          );
        }
      } catch (error) {
        console.error("Error:", error);
        showToast("Error", translations[lang].loginServerError, "danger");
  ...
  registerForm.addEventListener("submit", async (e) => {
    ...
    if (!registerName.value.trim()) {
      showError(registerNameError, translations[lang].nameRequiredError);
      isValid = false;
    }
    ...
    if (!registerPassword.value.trim()) {
      ...
      showError(registerPasswordError, "La contraseña debe tener al menos 8 caracteres");
      isValid = false;
    }
    ...
    if (isValid) {
      try {
        const response = await fetch("/dashboard/utils/admin/register", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (response.ok && data.success) {
          showToast(
            translations[lang].registerSuccess,
            translations[lang].registerSuccess,
            "success",
          );
          setTimeout(() => {
          ...
          let toastMsg = translations[lang].registerError;
          let toastType = "danger";
          if (response.status === 404) {
            toastMsg = translations[lang].registerNotAuthorized;
          } else if (response.status === 500) {
            toastMsg = translations[lang].registerServerError;
          }
          showToast("Error", data.message || toastMsg, toastType);
          showError(registerEmailError, data.message || toastMsg);
        }
      } catch (error) {
        console.error("Error:", error);
        showToast("Error", translations[lang].registerServerError, "danger");
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    if (lockoutUntil && Date.now() < parseInt(lockoutUntil)) {
      ...
      showError(
        loginPasswordError,
        `Demasiados intentos fallidos. Espere ${remainingTime} minutos.`,
      );
      return;
    }
  ...
  if (lockoutUntil && Date.now() < parseInt(lockoutUntil)) {
    const remainingTime = Math.ceil((parseInt(lockoutUntil) - Date.now()) / (60 * 1000));
    warningAlert.innerHTML += `<br><small>Su cuenta está bloqueada por ${remainingTime} minutos más.</small>`;
  }
});

// Mejora: Mostrar/ocultar contraseña con tecla Enter
document.addEventListener("keydown", function (e) {
  if (e.key === "Enter" && e.target.type === "password") {
    const toggleBtn = e.target.parentElement.querySelector('[id^="toggle-"]');
    if (toggleBtn) {
      toggleBtn.click();
    }
  }
