 ...
 * Files are stored in organized directories based on upload date and user ID.
 */
const storage: multer.StorageEngine = multer.diskStorage({
  /**
   * Determines the destination directory for uploaded files.
   * @param req - Express request object
   * @param _file - Uploaded file (not used here)
   * @param cb - Callback function to return the destination path
   */
  destination: (req, _file, cb): void => {
    try {
      // Use userId from query params or default to "unknown"
      const userId = req.query.userId?.toString() || "unknown";
      const date = new Date().toISOString().split("T")[0]; // YYYY-MM-DD format

      // Determine base path from config or fallback to relative path
      const basePath = config.project.cdnpath
        ? path.resolve(config.project.cdnpath)
        : path.join(__dirname, "../../../../../../config/backups/cdn-client"); // Safer relative path

      // Create full upload path with date and userId
      const uploadPath = path.join(basePath, `${date}_${userId}`);
   ...
   * @param _req - Express request object (not used here)
   * @param file - Uploaded file object
   * @param cb - Callback function to return the filename
   */
  filename: (_req, file, cb): void => {
    try {
      const uniqueSuffix = `${Date.now()}-${Math.round(Math.random() * 1e9)}`;
      const ext = path.extname(file.originalname);
      const sanitizedName = file.fieldname.replace(/[^a-zA-Z0-9-_]/g, "");
...
/**
 * Configured Multer middleware instance with:
 * - Custom storage engine
 * - File size limit (100MB)
 * - No file filter (accepts all files)
 */
...
export default ({ app }: TRoutesInput) => {
  ...
  app.post(formatRoute("auth/register"), async (req: Request, res: Response) => {
    try {
      ...
      req.login(createdUser, (err) => {
        if (err) {
          // Handle error
          return res.status(500).json({ success: false, message: "The error login session" });
        }
  ...
  app.get("/dashboard/utils/backups/get", async (req: Request, res: Response) => {
    try {
      const file = req.query.file as string;
      if (!file) {
        return res.status(400).json({ success: false, message: "File parameter is required" });
      }
      ...
      res.status(500).json({ success: false, message: "Error reading backup file" });
    }

    return;
  });

  app.post("/dashboard/utils/backups/create", async (req: Request, res: Response) => {
    try {
      const { description, compress } = req.body;
      const backupsDir = path.join(__dirname, "../../../../../config/backups");
      ...
      const backupData = {
        userAPI: await main.prisma.userAPI.findMany(),
        license: await main.prisma.license.findMany(),
        blockedIP: await main.prisma.blockedIP.findMany(),
        failedAttempt: await main.prisma.failedAttempt.findMany(),
        rateLimitViolation: await main.prisma.rateLimitViolation.findMany(),
        task: await main.prisma.task.findMany(),
        whatsApp: await main.prisma.whatsApp.findMany(),
        myDiscord: await main.prisma.client.findMany(),
        myGuild: await main.prisma.myGuild.findMany(),
        banUser: await main.prisma.banUser.findMany(),
        userWarn: await main.prisma.userWarn.findMany(),
        serverModlog: await main.prisma.serverModlog.findMany(),
        reminder: await main.prisma.reminder.findMany(),
        economy: await main.prisma.economy.findMany(),
        userInventory: await main.prisma.userInventory.findMany(),
        shopEconomy: await main.prisma.shopEconomy.findMany(),
        userEconomy: await main.prisma.userEconomy.findMany(),
        userDiscord: await main.prisma.userDiscord.findMany(),
        levelConfig: await main.prisma.levelConfig.findMany(),
        userLevel: await main.prisma.userLevel.findMany(),
        streak: await main.prisma.streak.findMany(),
        userQuests: await main.prisma.userQuests.findMany(),
        userAchievements: await main.prisma.userAchievements.findMany(),
        prestigeHistory: await main.prisma.prestigeHistory.findMany(),
        weeklyLeaderboard: await main.prisma.weeklyLeaderboard.findMany(),
        monthlyLeaderboard: await main.prisma.monthlyLeaderboard.findMany(),
        leaderboardReset: await main.prisma.leaderboardReset.findMany(),
        captcha: await main.prisma.captcha.findMany(),
        suggestion: await main.prisma.suggestion.findMany(),
        reactionRole: await main.prisma.reactionRole.findMany(),
      ...
      // Comprimir si se solicita (opcional)
      if (compress) {
        // Implementar lógica de compresión si es necesaria
      }
  ...
  app.delete("/dashboard/utils/backups/delete", async (req: Request, res: Response) => {
    try {
      const file = req.query.file as string;
      if (!file) {
        return res.status(400).json({ success: false, message: "File parameter is required" });
