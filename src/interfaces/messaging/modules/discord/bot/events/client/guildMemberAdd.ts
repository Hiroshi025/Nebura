...
export default createEvent({
  ...
  async run(member) {
    ...
    await member
      .send({
        embeds: [
          new EmbedCorrect().setTitle(t("captcha.instructionsTitle")).setDescription(
    ...
    collector?.on("collect", async (msg) => {
      ...
      if (memberInput === captcha.text?.toLowerCase()) {
        ...
        if (attempts >= 3) {
          member.send({
            embeds: [new EmbedCorrect().setDescription(`${messages.failed} ${t("captcha.kickWarning")}`)],
          });
          setTimeout(() => {
          ...
          member.send({
            embeds: [
              new EmbedCorrect()
                ...
                .setDescription(t("captcha.failedDescription", { attempts, maxAttempts: 3 })),
            ],
          });
        }
      }
